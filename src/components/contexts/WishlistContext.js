...
export const WishlistProvider = ({ children }) => {
    ...
    const fetchWishlist = async () => {
        if (isAuthenticated) {
            try {
                const response = await fetch(ApiService.wishlist.userWishlist(user.userId));
                if (!response.ok) {
                    setIsLoading(false);
    ...
    const handleRemove = async (productId) => {
        try {
            const response = await fetch(ApiService.wishlist.removeFromWishlist(user.userId, productId), {
                method: 'DELETE',
            });
            if (!response.ok) {
